...
# Register the googlesheet for storing data
surveyresults <- gs_key('1XdEisuvIo5EAD0mH0wMTwAWd8oatmlVRY4fQummQ918')
  
# Function for storing google data
save_data_gsheets <- function(data) {
    surveyresults <- surveyresults %>% gs_add_row(input = data)
}
   ...
   sidebarLayout(
      sidebarPanel(
        ...
        #Survey Questions on Side Panel
                selectInput("select_gender", label = h5("Gender"), 
        choices = list("Female" = 1, "Male" = 2, "Other" = 3), 
        selected = 1),
        selectInput("select_race", label = h5("Race"), 
        choices = list("American Indian" = 1, "Asian" = 2, "Black" = 3,
                       "Hispanic" = 4, "Multiracial" = 5, "White" = 6,
                       "Other" = 7), 
        selected = 1),
        sliderInput("slider1_age", label = h5("Age"), min = 18, 
        max = 99, value = 60),
        selectInput("select_income", label = h5("Gross Annual Income"), 
        ...
        selected = 1),
        selectInput("select_edu", label = h5("Highest Level of Education"), 
        ...
        selected = 1),
        sliderInput("slider1_polideo", label = div(h5("Political Ideology"), style='width:235px;',
...
server <- function(input, output, session){
  get_followers_ideology <- reactive({
    ...
    followers_in_set
    surveydata <- c(input$term, 
                    input$select_gender, 
                    input$select_race,
                    input$slider1_age,
                    input$select_income,
                    input$select_edu, 
                    input$slider1_polideo)
   save_data_gsheets(surveydata)
  })
